...
| 🟡 | Auth | Hash PINs / migrate to passwords with rate limiting and proper sessions | Admin + auth endpoints hash PINs and hide secrets; login auto-upgrades legacy plaintext. Rate limiting and http-only server sessions now live (`src/routes/api/auth/login/+server.js:1`, `src/lib/server/loginRateLimit.js:1`, `src/lib/server/session.js:1`). Next: script dormant-account rehash, migrate to stronger credentials, and enforce session-derived IDs across APIs. |
